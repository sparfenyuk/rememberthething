...
<script>
  ...
  function renderComposition(target, result) {
    ...
    const choices = unresolved.map((choice) => `<div class="hint"><p><strong>${escapeHtml(choice.label)}</strong><br>${escapeHtml(choice.module_name)} · choose one</p><span class="actions">${choice.options.map((option) => `<button data-choice="${escapeHtml(choice.choice_key)}" data-option="${escapeHtml(option.option_key)}" data-selection="${escapeHtml(choice.selection_id)}">${escapeHtml(option.name)}</button>`).join('')}</span></div>`).join('');
